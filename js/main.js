...
$.getJSON(DATA_DIR+current_datsel.properties,function(data){current_datsel.props=data;console.log('~~ Member properties of "'+current_datsel.id+'" have been loaded')})}var iS=current_datsel.datasets[dsi].options.initSelection;if(initComplete&&iS!==undefined){// set to datasets default selection
timeSel.data.x={min:iS.min,max:iS.max}}if(current_datsel.datasets[dsi].data!==undefined){current_setsel=current_datsel.datasets[dsi];updateUI();genChart();genGrid()}else{
// loading feedback
var l=0;var lAnim=setInterval(function(){var txt=["loading... &nbsp; &nbsp; ┬──┬","loading... &nbsp; &nbsp; ┬──┬","loading... (°o°） ┬──┬","loading... &nbsp;(°o°）┬──┬","loading... (╯°□°）╯ ┻━┻","loading... (╯°□°）╯︵ ┻━┻","loading... (╯°□°）╯︵ ︵ ┻━┻","loading... (╯°□°）╯︵ ︵ ︵ ┻━┻","loading... ︵ ︵ ︵ ┻━┻","loading... ︵ ︵ ┻━┻","loading... ︵ ┻━┻","loading... &nbsp; &nbsp; ┻━┻ &nbsp; &nbsp; (ツ)","loading... &nbsp; &nbsp; ┻━┻ &nbsp; &nbsp;(ツ)","loading... &nbsp; &nbsp; ┻━┻ &nbsp; (ツ)","loading... &nbsp; &nbsp; ┻━┻ &nbsp;(ツ)","loading... &nbsp; &nbsp; ┬──┬ ¯\\_(ツ)","loading... &nbsp; &nbsp; ┬──┬ (ツ)"];$("#legend").html(txt[l]);l=(l+1)%17},180);var lBM=new Date;console.log("~~ starting to load dataset "+current_datsel.datasets[dsi].strings.label+" ~~ ");$.getJSON(DATA_DIR+current_datsel.datasets[dsi].file,function(data){console.log(" |BM| finished loading "+current_datsel.datasets[dsi].strings.label+" data (took "+(new Date-lBM)+"ms)");current_datsel.datasets[dsi].data=data;current_setsel=current_datsel.datasets[dsi];lBM=Date.now();preprocess(current_setsel);console.log(" |BM| finished preprocessing object iterators ("+(Date.now()-lBM)+"ms)");clearInterval(lAnim);updateUI();genChart();initComplete=true;genGrid()})}}function preprocess(ds){if(ds.ready===true){return false}if(ds.itarraytor===undefined){ds.itarraytor=[]}var i=(C_WMAX-C_WMIN)/ds.parent.tile_width;while(i--){if(ds.itarraytor[i]===undefined){ds.itarraytor[i]=[];for(var k in ds.data[i]){ds.itarraytor[i].push(parseInt(k))}}}ds.ready=true}////////////////////
/// global scope ///
////////////////////
////////////////////
/// pseudo constants
...
/// DATA
var DATA_DIR="./data/",META_FILES=["humans.json","queries.json"];// available datagroups
...
$(function(){
// init stuff
lastMapCenter={lat:0,lng:0};resoFactor=parseFloat($("#reso-slider").val());
//$("#zoom-slider").attr("min",M_ZOOM_RANGE[0]).attr("max",M_ZOOM_RANGE[1]);
$bubble=$("#bubble");
// setup overlay canvas
overcan=d3.select("#map").append("canvas").classed("overlay",true);overctx=overcan.node().getContext("2d");
// init color scale
colorScale=d3.scale.log().range(M_COLOR_SCALE);
/// load all the meta data meta files
var bmMETA=new Date;console.log("~~ started loading the meta files (total of "+META_FILES.length+") ~~ ");var callback=function(data){for(var j=0;j<data.datasets.length;j++){data.datasets[j].parent=data}gdata[mfc++]=data;if(mfc===META_FILES.length){console.log(" |BM| got all the meta files (took "+(new Date-bmMETA)+"ms)");setupControlHandlers();onResize();statifyUrl()}};var mfc=0;// meta file counter
for(var i=0;i<META_FILES.length;i++){$.getJSON(DATA_DIR+META_FILES[i],callback)}});
////////////////
/// on resize //
////////////////
function onResize(){
// get new viewport size
viewportW=$(window).width();viewportH=$(window).height();var mapcbox=$("#controls-map"),tlcbox=$("#controls-timeline");if(viewportH<700){mapcbox.children("h2").addClass("closed");mapcbox.children("fieldset").slideUp()}else{mapcbox.children("h2").removeClass("closed");mapcbox.children("fieldset").slideDown()}if(viewportH<777){tlcbox.children("h2").addClass("closed");tlcbox.children("fieldset").slideUp()}else{tlcbox.children("h2").removeClass("closed");tlcbox.children("fieldset").slideDown()}
...
canvasW=Math.floor($("#map").width());canvasH=Math.floor($("#map").height());overcan.attr("width",canvasW).attr("height",canvasH);$("#leaflet").css("width",canvasW).css("height",canvasH);initLeaflet();genGrid();genChart()}/////////////////////////
/// general map logic ///
/////////////////////////
/**
** grid generation and 
...
clearTimeout(redrawTimer);redrawTimer=setTimeout(function(){generateGrid(reso,mAE,data)},CALC_TIMEOUT)}/**
* Generate data grid for the map
* all parameters optional */
function generateGrid(reso,mAE,data){if(!allow_redraw){return false}if(data===undefined){data=current_setsel}if(mAE===undefined){mAE=getBounds(true)}if(reso===undefined){reso=calcReso()}
/// main function body
var worker=function(){console.log("/~~ generating new grid with resolution "+reso+" ~~\\");var bms=new Date;var progBM="";console.log("  ~ start iterating data");var count=0;cellmap={};// reset cellmap
/// calculate everything we can outside the loop for performance
setColorScale(reso);
// get axisExtremes
var cAE=getTimeSelection();
// min and max tile
var mmt=getMinMaxTile(mAE);var tMin=mmt.min,tMax=mmt.max;console.log("  # will iterate over tiles "+tMin+" to "+tMax);
...
console.log("  |BM| iteration complete ("+(new Date-bms)+"ms)");calcPlotDat(cellmap,reso);console.log("  |BM| finished genGrid (total of "+(new Date-bms)+"ms)");var term=data.strings.term||L_DEFAULT_TERM;$("#legend").html("<em>inside the visible area</em><br>"+
//"<span>["+mAE[0].min.toFixed(1)+","+mAE[1].min.toFixed(1)+"]-["+mAE[0].max.toFixed(1)+","+mAE[1].max.toFixed(1)+"]</span><br>"+
"we have registered a total of<br>"+"<em>"+count+" "+data.strings.label+"</em><br>"+term.replace("%l","<em>"+cAE.min+"</em>").replace("%h","<em>"+cAE.max+"</em>"));selectCell();// urlifyState(); // is always called in selectCell
console.log("\\~~ grid generation complete~~/ ");mutexGenGrid=0;leaflaggrid._redraw()};
/// iterate & aggregate over a single tile
var iterate=function(offset){if(mutexGenGrid===-1){// cancel loop
mutexGenGrid=0;// if new function call
return 0}var cellmapprog={},a,ti,i,j,k,it;if(!renderRTL){i=tMin+offset}else{i=tMax-offset}if(i<=tMax&&i>=tMin){// still work to do	
it=data.itarraytor[i].length;// for each map tile in visible area
while(it--){j=data.itarraytor[i][it];// go over each key
if(j>=cAE.min&&j<=cAE.max){// if it is in selection range
k=data.data[i][j].length;while(k--){// go over each event in key and
...
calcPlotDat(cellmapprog,reso,i);leaflaggrid._redraw();setTimeout(function(){iterate(++offset)},1)}else{// iterated over all tiles
finish()}};iterate(0)};
// set it all in motion - if no other generation is running
if(mutexGenGrid===1){mutexGenGrid=-1}var goOn=function(){if(mutexGenGrid!==0){setTimeout(goOn,10)}else{mutexGenGrid=1;$("#legend").html("<em>massive calculations...</em>");setTimeout(worker,1)}};goOn()}
////////////////
/// filters ///
//////////////
...
function calcPlotDat(newmap,reso,tile){if(newmap!==undefined&&reso===undefined){conslole.warn("drawPlot(): newmap given but no resolution. Using old drawing data.")}var uMBM=Date.now();if(newmap!==undefined&&reso!==undefined){// calculate new drawing map
var draw=[],min=Infinity,max=-Infinity;$.each(newmap,function(k,v){var c=index2canvasCoord(k,reso);v=v.length;draw.push([c,v]);
// get extreme values
if(v<min){min=v}if(v>max){max=v}});drawdat={draw:draw,min:min,max:max,reso:reso}}drawdat.tile=tile;if(tile===undefined){console.log("  ~ drawing "+drawdat.draw.length+" shapes");console.log("  # data extreme values - min: "+drawdat.min+", max: "+drawdat.max);console.log("  |BM| (dataset generation in "+(new Date-uMBM)+"ms)")}}/**
* mapgrid drawing function hooked to leaflet */
function drawPlot(leavas,params){if(drawdat.draw===undefined){return false}var bm=Date.now();var mapctx=params.canvas.getContext("2d");
// for compensating leaflets css transformation
var transform=$(params.canvas).css("transform");transform=transform.split(",");var curAgPos=[parseInt(transform[transform.length-2]),// x
...
var bleed=parseFloat($("#bleed-slider").val()),wx=drawdat.reso*bleed,rx=wx/2,rr=drawdat.reso/2;drawdat.wx=wx;drawdat.rx=rx;drawdat.rr=rr;mapctx.save();if(drawdat.tile===undefined){mapctx.clearRect(0,0,params.canvas.width,params.canvas.height)}else if(typeof drawdat.tile==="number"){clearTile(drawdat.tile)}
// apply transparency when overlapping
var galph=1-(bleed-1)*.15;if(galph>1){galph=1}mapctx.globalAlpha=galph;var i=drawdat.draw.length,d,cx,cy;while(i--){d=drawdat.draw[i];cx=d[0].x-dx+rr;cy=d[0].y-dy+rr;mapctx.fillStyle=colorScale(d[1]);mapctx.beginPath();mapctx.arc(cx,cy,rx,0,TPI);mapctx.fill()}if(drawdat.tile===undefined){console.log("  |BM| canvas rendering of "+drawdat.draw.length+" shapes took "+(Date.now()-bm)+"ms")}mapctx.globalAlpha=1;mapctx.restore();lastAgPos=curAgPos}function highlightCell(c){var x,y,p,linewidth=2;var wx=drawdat.wx,rx=drawdat.rx,rr=drawdat.rr;overctx.save();overctx.clearRect(0,0,canvasW,canvasH);
// highlight the currently selected cell
if(selectedCell!==false){p=index2canvasCoord(selectedCell,drawdat.reso);x=p.x+rr;y=p.y+rr;
// fill the point
overctx.fillStyle="rgba(255,120,0,0.8)";overctx.beginPath();overctx.arc(x,y,rx,0,TPI);overctx.fill();
// draw 2 strokes around it
overctx.strokeStyle="rgba(255,255,255,0.4)";overctx.lineWidth=3*linewidth;overctx.beginPath();overctx.arc(x,y,rx*1.5,0,TPI);overctx.stroke();overctx.strokeStyle="rgba(0,0,0,0.5)";overctx.lineWidth=1*linewidth;overctx.beginPath();overctx.arc(x,y,rx*1.5,0,TPI);overctx.stroke()}if(c===false){overctx.restore();return false}
// highlight the currently hovered cell
p=index2canvasCoord(c,drawdat.reso);x=p.x+rr;y=p.y+rr;
// glow circle
var gradient=overctx.createRadialGradient(x,y,0,x,y,wx*2);gradient.addColorStop(0,"rgba(255,255,255,0");gradient.addColorStop(.25,"rgba(255,255,255,0.5");gradient.addColorStop(1,"rgba(255,255,255,0");overctx.fillStyle=gradient;overctx.beginPath();overctx.arc(x,y,rx*4,0,TPI);overctx.fill();
// inner fill
overctx.fillStyle="rgba(255,244,10,0.9)";overctx.beginPath();overctx.arc(x,y,rx,0,TPI);overctx.fill();overctx.restore()}/**
 * highlight all cells that contain data in key */
function highlightCellsFor(key){var data=current_setsel.data,reso=drawdat.reso,rx=drawdat.rx,rr=drawdat.rr;
//var bm = Date.now();
drawWhat();var mAE=drawdat.bounds,mmt=drawdat.mmt;/* supposed to be better and more reliable
	// but actually slower..
	var k, l, ca = [];
...
var i,t,j,ci,cmap={},ca=[];for(i=mmt.min;i<=mmt.max;i++){if((t=data[i][key])!==undefined){j=t.length;while(j--){if(section_filter(t[j],mAE)){ci=coord2index(t[j][ARR_M_LON],t[j][ARR_M_LAT],reso);cmap[ci]=true}}}}$.each(cmap,function(k){ca.push(k)});overctx.save();overctx.clearRect(0,0,canvasW,canvasH);overctx.fillStyle="rgba(255,255,255,0.7)";overctx.strokeStyle="rgba(0,0,0,1)";overctx.lineWidth=rx/2;var x,y,p;i=ca.length;while(i--){p=index2canvasCoord(ca[i],reso);x=p.x+rr;y=p.y+rr;overctx.beginPath();overctx.arc(x,y,rx,0,TPI);overctx.fill();overctx.stroke()}overctx.restore()}function initLeaflet(){if(leafly!==undefined){leafly.invalidateSize();return false}leafly=L.map("leaflet",{
//maxBounds: [[-90,-180],[90,180]],
attributionControl:false,worldCopyJump:true}).setView([0,0],2);leafly.___eventHandlersAtached=false;leafloor=L.tileLayer("",{
//noWrap: true,
subdomains:["a","b","c","d"],minZoom:M_ZOOM_RANGE[0],maxZoom:M_ZOOM_RANGE[1]});leafloor.addTo(leafly);L.control.attribution({prefix:false}).addAttribution('<a id="home-link" target="_top" href="http://maps.stamen.com/">Map tiles</a> by '+'<a target="_top" href="http://stamen.com">Stamen Design</a>, '+'under <a target="_top" href="http://creativecommons.org/licenses/by/3.0">CC BY 3.0</a>. '+'Data by <a target="_top" href="http://openstreetmap.org">OpenStreetMap</a>, '+'under <a target="_top" href="http://creativecommons.org/licenses/by-sa/3.0">CC BY SA</a>.').addTo(leafly);leaflaggrid=L.canvasOverlay();leaflaggrid.drawing(drawPlot).addTo(leafly)}function attachMapHandlers(){if(leafly.___eventHandlersAtached===true){console.warn("attachMapHandlers() has been called again.");return false}leafly.on("movestart",function(){selectCell(false)}).on("moveend",function(){
//clearGrid();
genGrid();genChart()}).on("zoomend",function(){drawdat.draw=undefined;clearGrid()}).on("mousemove",function(e){canvasMouseMove(e)}).on("click",function(e){canvasMouseClick(e)});leafly.___eventHandlersAtached=true}function changeTileSrc(){if(leafloor===undefined){return false}var upre="http://{s}.sm.mapstack.stamen.com/",usuf="/{z}/{x}/{y}.png",parm="(",url;$("#ctrl-maplayer input[type=checkbox]").each(function(){if(this.checked){if(parm!=="("){parm+=","}parm+=$(this).attr("data-str")}});if(parm==="("){url=""}else{url=upre+parm+")"+usuf}leafloor.setUrl(url)}/////////////////////
/// map utilities ///
/////////////////////
/***
...
function calcReso(){return canvasW/M_BASE_GRIDROWS*resoFactor}/**
...
function drawWhat(){if(drawdat.bounds===undefined){drawdat.bounds=getBounds(true)}if(drawdat.mmt===undefined){drawdat.mmt=getMinMaxTile(drawdat.bounds)}}/**
...
function getBounds(enforce){var b=leafly.getBounds();var bounds=[{min:b._southWest.lng,max:b._northEast.lng},{min:b._southWest.lat,max:b._northEast.lat}];
// boundary enforcement
if(enforce===true){if(bounds[0].min<C_WMIN){bounds[0].min=C_WMIN}if(bounds[0].min>C_WMAX){bounds[0].min=C_WMAX}if(bounds[0].max<C_WMIN){bounds[0].max=C_WMIN}if(bounds[0].max>C_WMAX){bounds[0].max=C_WMAX}if(bounds[1].min<C_HMIN){bounds[1].min=C_HMIN}if(bounds[1].min>C_HMAX){bounds[1].min=C_HMAX}if(bounds[1].max<C_HMIN){bounds[1].max=C_HMIN}if(bounds[1].max>C_HMAX){bounds[1].max=C_HMAX}}return bounds}/**
* returns min and max tile index for given or current bounds */
function getMinMaxTile(mAE){if(mAE===undefined){mAE=getBounds(true)}var tMin=0;while(mAE[0].min>C_WMIN+(tMin+1)*current_datsel.tile_width){tMin++}var tMax=tMin;while(mAE[0].max>C_WMIN+(tMax+1)*current_datsel.tile_width){tMax++}return{min:tMin,max:tMax}}/**
* returns the index value for the cell of a grid with given resolution
* where the given geocoordinate pair lies in*/
function coord2index(longi,lati,reso){var proj=leafly.latLngToContainerPoint(L.latLng(lati,longi));return Math.floor(proj.y/reso)*(canvasW/reso)+Math.floor(proj.x/reso)}/**
* returns the index value for given canvas (container) coordinates */
function canvasCoord2index(p,reso){return Math.floor(p.y/reso)*(canvasW/reso)+Math.floor(p.x/reso)}/**
* returns the geo coordinates for a given index
* (for the bottom left corner of the gridcells rect)*/
function index2geoCoord(i,reso){var cc=index2canvasCoord(i,reso);var gc=leafly.containerPointToLatLng([cc.x,cc.y]);return[gc.lat,gc.lng]}/**
* returns the canvas rendering coordinates for a given index 
* (for the top left corner of the gridscells rect) */
function index2canvasCoord(i,reso){var cpr=canvasW/reso;return{x:i%cpr*reso,y:Math.floor(i/cpr)*reso}}/**
* wipe it clean */
function clearGrid(){var ctx=leaflaggrid._canvas.getContext("2d");ctx.clearRect(0,0,canvasW,canvasH)}/**
* clears the area of tile i on map canvas */
function clearTile(i){var ctx=leaflaggrid._canvas.getContext("2d");var bounds=getBounds();var nw=L.latLng(bounds[1].max,current_datsel.tile_width*i),se=L.latLng(bounds[1].min,current_datsel.tile_width*(i+1));nw=leafly.latLngToContainerPoint(nw);se=leafly.latLngToContainerPoint(se);ctx.clearRect(nw.x,nw.y,se.x-nw.x,nw.y-se.y)}/**
* adjust the currently pointed at container coordinates */
function currentCursorPos(e){var p=e.containerPoint;return{x:p.x-M_HOVER_OFFSET.l,y:p.y-M_HOVER_OFFSET.t}}/**
* map tooltip */
function canvasMouseMove(e){if(mutexGenGrid!==0){return false}// don't bother while working hard
var cc=currentCursorPos(e);var i=canvasCoord2index(cc,drawdat.reso);var cell=cellmap[i];
// hover highlight
highlightCell(i);
//$("#hud").text('(' + cc.x.toFixed(5) + ', ' + cc.y.toFixed(5) + ')');
if(cell!==undefined){var p=index2geoCoord(i,drawdat.reso);var x=(p[0]+drawdat.reso/2).toFixed(2),y=(p[1]+drawdat.reso/2).toFixed(2);
// display the info bubble
clearTimeout(bubbleTimer);bubbleTimer=true;$bubble.css("opacity","1").css("bottom",viewportH-e.originalEvent.pageY+M_BUBBLE_OFFSET+"px").css("right",viewportW-e.originalEvent.pageX+M_BUBBLE_OFFSET+"px").html(cell.length+" <em>"+current_setsel.strings.label+"</em><br>"+"<span>["+x+", "+y+"]</span>")}else if(bubbleTimer===true){
// hide the info bubble
//highlightCell(false);
bubbleTimer=setTimeout(function(){$bubble.css("opacity","0")},250)}}function canvasMouseClick(e){if(mutexGenGrid!==0){return false}// don't bother while working hard
var cc=currentCursorPos(e);var i=canvasCoord2index(cc,drawdat.reso);selectCell(i)}/**
* select cell i and fill the infolist table */
function selectCell(i){if(drawdat.draw===undefined){return false}if(i===undefined){i=selectedCell}var $tb=$("#infolist"),$info=$("#cellinfo-desc>div");$tb.html("");// clear the list
if(i===false){$info.hide();selectedCell=false;highlightCell(false);urlifyState();return true}var cell=cellmap[i];if(cell===undefined){selectCell(false)}else{selectedCell=i;$info.show();var timeout=0;if(cell.length>100){$info.html("<em>assembling list...</em>");timeout=5}setTimeout(function(){var p=index2geoCoord(i,drawdat.reso);var x=(p[0]+drawdat.reso/2).toFixed(2),y=(p[1]+drawdat.reso/2).toFixed(2);highlightCell(i,true);$tb.html("");
// TODO recursive timeouts for large arrays (around >5000)
$.each(cell,function(){var q=current_datsel.props.members[this[0]];$tb.append("<tr>"+'<td><a class="q" href="https://www.wikidata.org/wiki/'+q+'" data-qid="'+q+'" target="wikidata">'+q+"</a></td>"+"<td>"+this[1]+"</td>"+"</tr>")});$info.html("the <em>selected cell</em> <span>around "+"("+x+", "+y+")</span> "+"contains <em>"+cell.length+"</em> of them:");$tb.trigger("scroll")},timeout);urlifyState()}}/**
* infolistScroll Timeout Wrapper*/
function infolistScroll(){clearTimeout(infolistTimer);infolistTimer=setTimeout(infolistScrollFkt,CALC_TIMEOUT)}/**
* infolist scroll handler, fetches labels for visible items */
function infolistScrollFkt(){var cellinfo=$("#cellinfo");var infolist=$("#infolist");var infolistT=cellinfo.position().top+infolist.position().top;var infolistB=infolistT+infolist.height();var qarray=[];var lang=$("#langsel").val();infolist.find("a.q").each(function(){var t=this.getBoundingClientRect().top;if(t>=infolistT&&t<=infolistB){qarray.push(this)}});
...
function setColorScale(r){if(r===undefined){r=calcReso()}
// values determined experimentally for now
// TODO find a way to automate this
var max=15e3*r,n=M_COLOR_SCALE.length-1,domain=[1];var e=Math.log(max);for(var i=1;i<=n;i++){domain.push(Math.floor(Math.pow(Math.E,e/n*i)))}colorScale.domain(domain)}////////////////
/// timeline ///
////////////////
/**
...
*/
function genChart(data){if(current_setsel.ready!==true){return false}var benchmark_chart=new Date;updateChartDataFkt(data);initChart();console.log("  |BM| chart creation complete (total of "+(new Date-benchmark_chart)+"ms)");console.log("\\~~ finished generating chart ~~/ ")}/**
* timeout wrapper * /
(not used right now)
...
function updateChartDataFkt(data){if(data===undefined){data=current_setsel}// TODO
console.log("/~~ generating chart data ~~\\ ");var benchmark_chart=Date.now();drawWhat();var mAE=drawdat.bounds,mmt=drawdat.mmt;chartdat=[];var globob={},locob={},i,j,l,k,d,it,itl;
/// create envision data from the assembled object
var nvision=function(o){
// create a sorted index for all o props
var x=[],y=[],kay=[];for(i in o){kay.push(parseInt(i))}kay.sort(function(a,b){return a-b});
// and iterate over it
l=kay.length;for(i=0;i<l;i++){x.push(kay[i]);y.push(o[kay[i]])}chartdat.push([x,y])};
...
var tilecount=(C_WMAX-C_WMIN)/data.parent.tile_width;for(i=0;i<tilecount;i++){itl=data.itarraytor[i].length;it=-1;
//if(i >= mmt.min && i <= mmt.max) { // in range, collect both
while(++it<itl){j=data.itarraytor[i][it];d=data.data[i][j];l=d.length;
// global part
if(globob[j]===undefined){globob[j]=d.length}else{globob[j]+=d.length}
// local part
if(locob[j]===undefined){locob[j]=0}if(i>=mmt.min&&i<=mmt.max){for(k=0;k<l;k++){d=data.data[i][j][k];if(section_filter(d,mAE)){locob[j]++}}}}}nvision(globob);nvision(locob)}else{// collect map area data only
for(i=mmt.min;i<=mmt.max;i++){it=data.itarraytor[i].length;while(it--){j=data.itarraytor[i][it];l=data.data[i][j].length;if(locob[j]===undefined){locob[j]=0}for(k=0;k<l;k++){d=data.data[i][j][k];if(section_filter(d,mAE)){locob[j]++}}}}nvision(locob)}console.log("  |BM| timeline data updated in "+(Date.now()-benchmark_chart)+"ms")}function initChart(){if(chartdat.length<=0){return false}if(chart!==undefined){chart.destroy()}var connectionH=10;// height of connection component
...
var range=getTimeSelection();if(timeSel.data.x.min!==range.min||timeSel.data.x.max!==range.max){timeSel.data.x={min:range.min,max:range.max};genGrid()}$("#range-tt-min>div").text(range.min);$("#range-tt-max>div").text(range.max)};var detail,detailOptions,summaryOptions,// summary, (is global)
...
colors:timelineIsGlobal?["#00A8F0","#C0D800"]:["#A0D345"],bars:{lineWidth:1,show:true,fill:true,fillOpacity:.6},mouse:{track:true,trackY:false,trackAll:true,sensibility:1,trackDecimals:4,position:"nw",lineColor:"#ff9900",fillColor:"#ff9900",fillOpacity:.6,trackFormatter:function(){var str=current_setsel.strings.timelineToolTip||T_DEFAULT_TOOLTIP;str=str.match(/(.*)(\%.)(.*)(\%.)(.*)(\%.)(.*)/);var fkt="var k = parseInt(o.x);"+"highlightCellsFor(k);"+'return "';for(var i=1;i<str.length;i++){if(str[i]==="%l"){fkt+="<em>"+current_setsel.strings.label+"</em> "}else if(str[i]==="%x"){fkt+='" + k + " '}else if(str[i]==="%v"){fkt+='<em>" + parseInt(o.y) + "</em> '}else{fkt+=str[i]}}
//.replace(/(\d)(?=(\d{3})+$)/g, "$1,")
fkt+=" <span>(";if(timelineIsGlobal){fkt+='<em>"+chart.components[0].options.config.data[1][o.index][1]+"</em> '}fkt+='in map area)</span>";';return Function("o",fkt)}()},yaxis:{autoscale:true,autoscaleMargin:.05,noTicks:4,showLabels:true,min:0}}};
...
// Wireup Interaction
interaction.leader(summary).follower(detail).follower(connection).add(envision.actions.selection,{callback:selCallback});appendTimelineRangeTips();appendListeners();
// set to initial selection state
summary.trigger("select",timeSel)}/**
* sets or changes the time selection */
function changeTimeSel(min,max,relative){if(relative===undefined){relative=true}var sel=chart.components[2].api.flotr.selection;if(relative){timeSel.fmin+=min-Math.floor(min);timeSel.fmax+=max-Math.floor(max);min=Math.floor(min+timeSel.data.x.min+Math.floor(timeSel.fmin));max=Math.floor(max+timeSel.data.x.max+Math.floor(timeSel.fmax));timeSel.fmin-=Math.floor(timeSel.fmin);timeSel.fmax-=Math.floor(timeSel.fmax)}if(min>max){return false}if(max>current_setsel.max){return false}if(min<current_setsel.min){return false}timeSel.data.x={min:min,max:max};if(min!==current_setsel.min||max!==current_setsel.max){sel.selecting=true}sel.setSelection({x1:min,x2:max});genGrid()}/**
* returns sanitized selection of the timeline */
function getTimeSelection(){var cAE,min=current_setsel.min,max=current_setsel.max,sel=chart.components[2].api.flotr.selection;if(sel.selecting!==false){cAE=sel.getArea();cAE.min=Math.round(cAE.x1);cAE.max=Math.round(cAE.x2)}else{cAE={min:min,max:max}}return{min:cAE.min>=min?cAE.min:min,max:cAE.max<=max?cAE.max:max}}function appendListeners(){$(chart.components[0].node).on("mouseleave",function(){highlightCell(false)})}function appendTimelineRangeTips(){var cont=$('<div id="range-tt-min" class="range-tt hover-tt">'+'<span class="arrow arrow-left"></span>'+"<div></div>"+'<span class="arrow arrow-right"></span>'+"</div>"+'<div id="range-tt-max" class="range-tt hover-tt">'+'<span class="arrow arrow-left"></span>'+"<div></div>"+'<span class="arrow arrow-right"></span>'+"</div>");//.hide();
$("#chart").append(cont);
// change interval limits with arrows
var repeatTimeout,repeatInterval;$(".range-tt .arrow").on("mousedown",function(){
//allow_redraw = false;
var min=$(this).parent().attr("id")==="range-tt-min"?$(this).hasClass("arrow-left")?-1:+1:0;var max=$(this).parent().attr("id")==="range-tt-max"?$(this).hasClass("arrow-left")?-1:+1:0;changeTimeSel(min,max);repeatTimeout=setTimeout(function(){repeatInterval=setInterval(function(){changeTimeSel(min,max)},100)},100)}).on("mouseup",function(){clearTimeout(repeatTimeout);clearInterval(repeatInterval)});
// drag behavior on detail view
var stepSize;var drag=d3.behavior.drag().on("drag",function(u,i){changeTimeSel(-d3.event.dx*stepSize,-d3.event.dx*stepSize)}).on("dragstart",function(){stepSize=(timeSel.data.x.max-timeSel.data.x.min)/$("#chart").width()});d3.selectAll("#chart .detail").call(drag)}//////////////////////
/// user interface ///
//////////////////////
/**
...
// build filter menu
var fn=function(){setSetSel(this.value,$(this).parent().parent().attr("data-gi"))};var filter=$("#filter");for(var i=0;i<gdata.length;i++){var fs=$("<fieldset>").attr("data-gid",gdata[i].id).attr("data-gi",i);fs.append("<legend>"+gdata[i].title+"</legend>");for(var j=0;j<gdata[i].datasets.length;j++){var b=$('<input type="radio" name="radio" value="'+j+'" />').on("change",fn);
//var tt = $('<span class="tooltip">'+gdata[i].datasets[j].strings.desc+'</span>');
fs.append($('<label class="tooltip" data-tt="'+gdata[i].datasets[j].strings.desc+'">'+gdata[i].datasets[j].strings.label+"</label>").prepend(b))}filter.append(fs)}$(".controls input[type='range']").on("input",function(){$(this).siblings("input[type='text']").val(parseFloat($(this).val()).toFixed(1))});/*$("#zoom-slider").on("change", function() {
		transitTo(getZoomTransform($(this).val()));
	});*/
$("#reso-slider").on("change",function(){resoFactor=parseFloat($(this).val());genGrid()});$("#bleed-slider").on("change",function(){leaflaggrid._redraw();urlifyState()});/*$("#freezer>input").on("change", function() {
...
$("#map-opacity").on("input",function(){$(leafloor._container).css("opacity",$(this).val());urlifyState()});$("#ctrl-maplayer input[type=checkbox]").on("change",function(){changeTileSrc();urlifyState()});$("#ctrl-tlmode input").on("change",function(){timelineIsGlobal=parseInt($(this).val());
//updateChartData();
genChart();urlifyState()});$("#sidebar>menu h2").on("click",function(){$(this).toggleClass("closed");$(this).siblings("fieldset").slideToggle()});$("#infolist").on("scroll",infolistScroll);$("#map").on("mouseleave",function(){$bubble.css("opacity","0")});
...
var langs=langCodes;for(i=0;i<langs.length;i++){$("#langsel").append($('<option value="'+langs[i]+'">'+langs[i]+"</option>"))}$("#langsel").on("change",function(){$("#infolist a").addClass("q");$("#infolist").trigger("scroll");urlifyState()})}/**
* update UI labels etc
* (so far only right column of item table) */
function updateUI(){var zprop=current_setsel.strings.zprop||T_DEFAULT_ZPROP;$("#cellinfo th:last-child").text(zprop)}
// TODO:  mapop sl
////////////////////////////////
...
function urlifyState(){if(!initComplete){return false}
// selected dataset
var setsel=$("#filter input[type='radio']:checked").val();var hash="d="+setsel;
// selected datagroup
...
// timeline selection
var time=getTimeSelection();hash+="&e="+time.min+"_"+time.max;
// selected cell
hash+="&c="+selectedCell;
// grid resolution
hash+="&g="+resoFactor;
...
var tileconf=0;if($("#maplayer-labels").get(0).checked){tileconf+=1}if($("#maplayer-shape").get(0).checked){tileconf+=2}hash+="&p="+tileconf;
// map transformation state
var mcenter=leafly.getCenter();hash+="&x="+mcenter.lng+"&y="+mcenter.lat;hash+="&z="+leafly.getZoom();window.location.hash=hash}/**
* restore the url encoded viz state */
function statifyUrl(){mutexStatify=1;var hash=window.location.hash;
//if (hash === "") { return false; }
/// default values
var labellang=DEFAULT_LABELLANG,timesel,dg=DEFAULT_DATAGROUP,ds=DEFAULT_DATASET,tl_mode=timelineIsGlobal,tile_opacity=M_DEFAULT_TILE_OPACITY,tile_conf=M_DEFAULT_TILE_CONF,map_lng=0,map_lat=0,map_zoom=2;hash=hash.substring(1).split("&");for(var i=0;i<hash.length;++i){var key=hash[i].substring(0,1);var val=hash[i].substring(2);switch(key){case"d":// dataset selection
ds=parseInt(val);break;case"l":// item label language
labellang=val;break;case"f":// timeline data source (global or map area)
tl_mode=val;break;case"m":// datagroup selection
dg=val;break;case"e":// time selection (envision)
var e=val.split("_");timesel={min:parseInt(e[0]),max:parseInt(e[1])};break;case"g":// grid resolution
$("#reso-slider").val(parseFloat(val)).trigger("input").trigger("change");break;case"h":// drawing overlap
$("#bleed-slider").val(parseFloat(val)).trigger("input");break;case"o":// tilemap opacity
tile_opacity=parseFloat(val);break;case"p":// tilemap parameters
tile_conf=parseInt(val);break;case"x":// map longitude
map_lng=parseFloat(val);break;case"y":// map latitude
map_lat=parseFloat(val);break;case"z":// map zoom
map_zoom=parseInt(val);break;case"c":// selected cell
selectedCell=parseFloat(val);break;default:console.warn("statifyUrl(): discarded unrecognized parameter '"+key+"' in url pattern")}}
// datagroup fallback
var dgi=gdata.length;while(--dgi&&gdata[dgi].id!==dg){}if(gdata[dgi].id!==dg){console.warn('cannot find datagroup "'+dg+'". '+'Falling back to "'+gdata[dgi].id+'"');dg=gdata[dgi].id}if(timesel===undefined){timesel={min:gdata[dgi].datasets[ds].options.initSelection.min,max:gdata[dgi].datasets[ds].options.initSelection.max}}
// label language
...
// time selection
timeSel={data:{// TODO this could go into dataset config options
x:{min:timesel.min,max:timesel.max}},fmin:0,fmax:0};
// recreate tilemap config
$("#map-opacity").val(parseFloat(tile_opacity)).trigger("input");$("#maplayer-shape").get(0).checked=tile_conf>1;$("#maplayer-labels").get(0).checked=tile_conf%2;changeTileSrc();
// recreate map state
leafly.setView([map_lat,map_lng],map_zoom,{reset:true});
// select dataset
$("#filter fieldset[data-gid="+dg+"] input").get(ds).click();attachMapHandlers()}